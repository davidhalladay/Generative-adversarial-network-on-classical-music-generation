...
def main():
    ...
    # load my Dataset
    train_dataset = Dataset(filepath = file_root,num = 4000)
    ...
    # models setting
    G_model = oneG_model.Generator()
    D_model = oneG_model.Discriminator()
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            ####################################################
            # self test
            print("num of non-zero :",np.sum(data[0].cpu().data.numpy() > 0.2))
        ...
        if (epoch) % 50 == 0:
            save_checkpoint('./save/GAN-G-oneG-%03i.pth' % (epoch) , G_model, G_optimizer)
            save_checkpoint('./save/GAN-D-oneG-%03i.pth' % (epoch) , D_model, D_optimizer)
        ...
        if (epoch) % 50 == 0:
            ...
            print(test_song)
            print("num of non-zero :",np.sum(test_song > 0.2))
            # torch.Size([1, 2, 384, 128])
            test_song = test_song.transpose(0, 2, 3, 1)
            tarck1 , tarck2 = make_mid(test_song, 0.2)
            print(tarck1)
            cc = Multitrack(tracks=[tarck1,tarck2], tempo=100.0,beat_resolution=24)
            cc.write('./save_songs/GAN/oneG_%03d' %(epoch+1))
