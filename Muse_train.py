...
def main():
    ...
    # models setting
    G_tmp_model = Musemodel.Generator_tmp()
    G_bar_model = Musemodel.Generator_bar()
    D_model = Musemodel.Discriminator()
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch) % 50 == 0:
            save_checkpoint('./save/MuseGAN-G-tmp-%03i.pth' % (epoch) , G_tmp_model, G_tmp_optimizer)
            save_checkpoint('./save/MuseGAN-G-bar-%03i.pth' % (epoch) , G_bar_model, G_bar_optimizer)
            save_checkpoint('./save/MuseGAN-D-%03i.pth' % (epoch) , D_model, D_optimizer)
        ...
        if (epoch) % 20 == 0:
            ...
            print(tarck1)
            cc = Multitrack(tracks=[tarck1,tarck2], tempo=120.0,beat_resolution=24)
            cc.write('./save_songs/GAN/Muse_%03d' %(epoch+1))
